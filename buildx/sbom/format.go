...
// Decoder is a function that can convert an SBOM document of a specific format from a reader into buildx native objects.
type Decoder func(reader io.Reader) (*SBOM, error)

// Encoder is a function that can transform buildx native objects into an SBOM document of a specific format written to the given writer.
...
// that the format requires. For example, all buildxjson formatted documents have a schema section which should have
// "metasources/buildx" within the version --if this isn't found then the validator should raise an error. These active
