...
import (
	...
	"github.com/metasources/buildx/buildx/formats/common"
...
func encodeComponent(p pkg.Package) cyclonedx.Component {
	props := encodeProperties(p, "buildx:package")
	props = append(props, encodeCPEs(p)...)
	locations := p.Locations.ToSlice()
	if len(locations) > 0 {
		props = append(props, encodeProperties(locations, "buildx:location")...)
	}
	if hasMetadata(p) {
		props = append(props, encodeProperties(p.Metadata, "buildx:metadata")...)
	}
...
func deriveBomRef(p pkg.Package) string {
	// try and parse the PURL if possible and append buildx id to it, to make
...
func decodeComponent(c *cyclonedx.Component) *pkg.Package {
	...
	common.DecodeInto(p, values, "buildx:package", CycloneDXFields)
...
func decodeLocations(vals map[string]string) source.LocationSet {
	v := common.Decode(reflect.TypeOf([]source.Location{}), vals, "buildx:location", CycloneDXFields)
...
func decodePackageMetadata(vals map[string]string, c *cyclonedx.Component, typ pkg.MetadataType) interface{} {
	if typ != "" && c.Properties != nil {
		...
		metaPtr := common.Decode(metaPtrTyp, vals, "buildx:metadata", CycloneDXFields)
