/*
Package buildx is a "one-stop-shop" for helper utilities for all major functionality provided by child packages of the buildx library.

Here is what the main execution path for buildx does:
...
// SetLogger sets the logger object used for all buildx logging calls.
func SetLogger(logger logger.Logger) {
	log.Log = logger
}

// SetBus sets the event bus for all buildx library bus publish events onto (in-library subscriptions are not allowed).
