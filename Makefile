...
unit: $(TEMP_DIR) fixtures  ## Run unit tests (with coverage)
	...
	go test -coverprofile $(TEMP_DIR)/unit-coverage-details.txt $(shell go list ./... | grep -v metasources/buildx/test)
...
cli: $(SNAPSHOT_DIR)  ## Run CLI tests
	...
	BUILDX_BINARY_LOCATION='$(SNAPSHOT_BIN)' \
...
fingerprints:
	...
	cd buildx/pkg/cataloger/binary/test-fixtures && \
		make cache.fingerprint

	# for JAVA BUILD test fixtures
	cd buildx/pkg/cataloger/java/test-fixtures/java-builds && \
		make packages.fingerprint

	# for GO BINARY test fixtures
	cd buildx/pkg/cataloger/golang/test-fixtures/archs && \
		make binaries.fingerprint

	# for RPM test fixtures
	cd buildx/pkg/cataloger/rpm/test-fixtures && \
		make rpms.fingerprint

	# for Kernel test fixtures
	cd buildx/pkg/cataloger/kernel/test-fixtures && \
		make cache.fingerprint
...
fixtures:
	...
	cd buildx/pkg/cataloger/java/test-fixtures/java-builds && make
	cd buildx/pkg/cataloger/rpm/test-fixtures && make
	cd buildx/pkg/cataloger/binary/test-fixtures && make
...
generate-compare-file:
	...
	go run ./cmd/buildx $(COMPARE_TEST_IMAGE) -o json > $(COMPARE_DIR)/test-fixtures/acceptance-centos-8.2.2004.json
