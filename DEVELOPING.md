...
4. for an even quicker start you can run `go run cmd/buildx/main.go` to print the buildx help.
	- this command `go run cmd/buildx/main.go alpine:latest` will compile and run buildx against `alpine:latest`
5. view the README or buildx help output for more output options
...
buildx is used to generate a Software Bill of Materials (SBOM) from different kinds of input.

### Code organization for the cmd package

buildx's entrypoint can be found in the `cmd` package at `cmd/buildx/main.go`. `main.go` builds a new buildx `cli` via `cli.New()` 
and then executes the `cli` via `cli.Execute()`. The `cli` package is responsible for parsing command line arguments, 
setting up the application context and configuration, and executing the application. Each of buildx's commands 
(e.g. `packages`, `attest`, `version`) are implemented as a `cobra.Command` in their respective `<command>.go` files. 
They are registered in `buildx/cli/commands/go`.
```
.
└── buildx/
    ├── cli/
...
```mermaid
sequenceDiagram
    participant main as cmd/buildx/main
...
### Code organization for buildx library

buildx's core library (see, exported) functionality is implemented in the `buildx` package. The `buildx` package is responsible for organizing the core
...
- package catalogers and their organization can be viewed/added to the `buildx/pkg/cataloger` package 
- file catalogers and their organization can be viewed/added to the `buildx/file` package
- The source package provides an abstraction to allow a user to loosely define a data source that can be cataloged

#### Code example of buildx as a library

Here is a gist of using buildx as a library to generate a SBOM for a docker image: [link](https://gist.github.com/wagoodman/57ed59a6d57600c23913071b8470175b).
The execution flow for the example is detailed below.

#### Execution flow examples for the buildx library

```mermaid
sequenceDiagram
    participant source as source.New(ubuntu:latest)
    participant sbom as sbom.SBOM
    participant catalog as buildx.CatalogPackages(src)
    participant encoder as buildx.Encode(sbom, format)
    ...
    sbom-->>encoder: pass SBOM and format desiered to buildx encoder
...
Catalogers are the way in which buildx is able to identify and construct packages given some amount of source metadata.
For example, buildx can locate and process `package-lock.json` files when performing filesystem scans. 
See: [how to specify file globs](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/cataloger/javascript/cataloger.go#L16-L21)
and an implementation of the [package-lock.json parser](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/cataloger/javascript/cataloger.go#L16-L21) for a quick review.
...
Catalogers must fulfill the interface [found here](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/cataloger.go). 
This means that when building a new cataloger, the new struct must implement both method signatures of `Catalog` and `Name`.

A top level view of the functions that construct all the catalogers can be found [here](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/cataloger/cataloger.go).
When an author has finished writing a new cataloger this is the spot to plug in the new catalog constructor.

For a top level view of how the catalogers are used see [this function](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/cataloger/catalog.go#L41-L100) as a reference. It ranges over all catalogers passed as an argument and invokes the `Catalog` method:

Each cataloger has its own `Catalog` method, but this does not mean that they are all vastly different.
Take a look at the `apkdb` cataloger for alpine to see how it [constructs a generic.NewCataloger](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/cataloger/apkdb/cataloger.go).

`generic.NewCataloger` is an abstraction buildx uses to make writing common components easier. First, it takes the `catalogerName` to identify the cataloger.
...
See [here for the APK example](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/apk_metadata.go#L16-L41).
- The other is a `parseFunction` which informs the cataloger what to do when it has found one of the above matches files.
See this [link for an example](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/cataloger/apkdb/parse_apk_db.go#L22-L102).
...
Identified packages share a common struct so be sure that when the new cataloger is constructing a new package it is using the [`Package` struct](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/package.go#L16-L31).

Metadata Note: Identified packages are also assigned specific metadata that can be unique to their environment. 
See [this folder](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg) for examples of the different metadata types.
...
- [Call for new package](https://github.com/metasources/buildx/blob/v0.70.0/buildx/pkg/cataloger/apkdb/parse_apk_db.go#L106)
- [APK Package Constructor](https://github.com/metasources/buildx/tree/v0.70.0/buildx/pkg/cataloger/apkdb/package.go#L12-L27)
...
All catalogers are provided an instance of the [`source.FileResolver`](https://github.com/metasources/buildx/blob/v0.70.0/buildx/source/file_resolver.go#L8) to interface with the image and search for files. The implementations for these 
...
- `integration`: located within `test/integration`, these tests focus on the behavior surfaced by the common library 
  entrypoints from the `buildx` package and make light assertions about the results surfaced. Additionally, these tests
...
- `cli`: located with in `test/cli`, these are tests that test the correctness of application behavior from a 
  ...
  CLI switches, and glue code before buildx library calls).
...
- `acceptance`: located within `test/compare` and `test/install`, these are smoke-like tests that ensure that application  
  ...
  output of a buildx invocation matches canned expected output. New acceptance tests should be added for each release artifact
...
func TestCatalogPackages(t *testing.T) {
  ...
  for _, test := range cases {
    t.Run(test.name, func (t *testing.T) {
      // use inputFixturePath and assert that buildx.CatalogPackages() returns the set of expected Package objects
      // ...
    })
  }
}
...
presenters. This list is updated and published periodically by an external group and buildx can grab and update this
