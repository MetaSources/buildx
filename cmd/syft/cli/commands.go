...
// New constructs the `buildx packages` command, aliases the root command to `buildx packages`,
// and constructs the `buildx power-user` command. It is also responsible for
// organizing flag usage and injecting the application config for each command.
// It also constructs the buildx attest command and the buildx version command.
...
func New() (*cobra.Command, error) {
	...
	cmds := []*cobra.Command{
		...
		attestCmd,
		Version(v, app),
		cranecmd.NewCmdAuthLogin("buildx"), // buildx login uses the same command as crane
	}
