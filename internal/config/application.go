...
import (
	...
	"github.com/metasources/buildx/buildx/pkg/cataloger"
	golangCataloger "github.com/metasources/buildx/buildx/pkg/cataloger/golang"
...
// Application is the main buildx application configuration.
type Application struct {
	// the location where the application config was read from (either from -c or discovered while loading); default .buildx.yaml
...
func loadConfig(v *viper.Viper, configPath string) error {
	...
	if _, err := os.Stat("config.yaml"); err == nil {
		log.Warn("DEPRECATED: ./config.yaml as a configuration file is deprecated and will be removed as an option in v1.0.0, please rename to .buildx.yaml")
	}
