...
func TestApplicationConfig(t *testing.T) {
	...
	// VALID: .buildx.yaml, .buildx/config.yaml, ~/.buildx.yaml, <XDG_CONFIG_HOME>/buildx/config.yaml
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(t *testing.T) string {
				return "./test-fixtures/.buildx.yaml"
		...
		{
			name: "current working directory buildx dir config",
			setup: func(t *testing.T) string {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// this will override home in case you are running this test locally and DO have a buildx config
